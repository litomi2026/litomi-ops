...
locals {
  initial_web_secret_content = base64encode(jsonencode({
    ...
    OTEL_EXPORTER_OTLP_ENDPOINT  = "REPLACE_ME_OTEL_EXPORTER_OTLP_ENDPOINT"
    OTEL_EXPORTER_OTLP_HEADERS   = "REPLACE_ME_OTEL_EXPORTER_OTLP_HEADERS"
  }))
  ...
  initial_api_secret_content = base64encode(jsonencode({
    ...
    APP_POSTGRES_CERTIFICATE            = "REPLACE_ME_APP_POSTGRES_CERTIFICATE"
    APP_POSTGRES_URL                    = "REPLACE_ME_APP_POSTGRES_URL"
    BBATON_CLIENT_ID                    = "REPLACE_ME_BBATON_CLIENT_ID"
    BBATON_CLIENT_SECRET                = "REPLACE_ME_BBATON_CLIENT_SECRET"
    CATALOG_POSTGRES_CERTIFICATE        = "REPLACE_ME_CATALOG_POSTGRES_CERTIFICATE"
    CATALOG_POSTGRES_URL                = "REPLACE_ME_CATALOG_POSTGRES_URL"
    CHAT_POSTGRES_CERTIFICATE           = "REPLACE_ME_CHAT_POSTGRES_CERTIFICATE"
    CHAT_POSTGRES_URL                   = "REPLACE_ME_CHAT_POSTGRES_URL"
    GA_PROPERTY_ID                      = "REPLACE_ME_GA_PROPERTY_ID"
    GOOGLE_APPLICATION_CREDENTIALS_JSON = "REPLACE_ME_GOOGLE_APPLICATION_CREDENTIALS_JSON"
    JWT_SECRET_ACCESS_TOKEN             = "REPLACE_ME_JWT_SECRET_ACCESS_TOKEN"
    JWT_SECRET_REFRESH_TOKEN            = "REPLACE_ME_JWT_SECRET_REFRESH_TOKEN"
    JWT_SECRET_TRUSTED_DEVICE           = "REPLACE_ME_JWT_SECRET_TRUSTED_DEVICE"
    ...
    OTEL_EXPORTER_OTLP_ENDPOINT         = "REPLACE_ME_OTEL_EXPORTER_OTLP_ENDPOINT"
    OTEL_EXPORTER_OTLP_HEADERS          = "REPLACE_ME_OTEL_EXPORTER_OTLP_HEADERS"
    PORTONE_API_SECRET                  = "REPLACE_ME_PORTONE_API_SECRET"
    PORTONE_CHANNEL_KEY                 = "REPLACE_ME_PORTONE_CHANNEL_KEY"
    PORTONE_STORE_ID                    = "REPLACE_ME_PORTONE_STORE_ID"
    PORTONE_WEBHOOK_SECRET              = "REPLACE_ME_PORTONE_WEBHOOK_SECRET"
  ...
  initial_recommender_secret_content = base64encode(jsonencode({
    APP_POSTGRES_CERTIFICATE     = "REPLACE_ME_APP_POSTGRES_CERTIFICATE"
    APP_POSTGRES_URL             = "REPLACE_ME_APP_POSTGRES_URL"
    CATALOG_POSTGRES_CERTIFICATE = "REPLACE_ME_CATALOG_POSTGRES_CERTIFICATE"
    CATALOG_POSTGRES_URL         = "REPLACE_ME_CATALOG_POSTGRES_URL"
    OPENSEARCH_PASSWORD          = "REPLACE_ME_OPENSEARCH_PASSWORD"
    OPENSEARCH_URL               = "REPLACE_ME_OPENSEARCH_URL"
    OPENSEARCH_USERNAME          = "REPLACE_ME_OPENSEARCH_USERNAME"
  }))
  ...
  initial_argocd_secret_content = base64encode(jsonencode({
    ...
    CLOUDFLARE_CACHE_PURGE_API_TOKEN       = "REPLACE_ME_CLOUDFLARE_CACHE_PURGE_API_TOKEN"
...
resource "oci_vault_secret" "recommender" {
  compartment_id = var.compartment_id
  description    = "Secret container for the recommender workload. Secret values are managed out-of-band."
  key_id         = oci_kms_key.this.id
  secret_name    = var.recommender_secret_name
  vault_id       = oci_kms_vault.this.id
  freeform_tags  = var.freeform_tags

  secret_content {
    content      = local.initial_recommender_secret_content
    content_type = "BASE64"
    name         = "initial-placeholder"
    stage        = "CURRENT"
  }
