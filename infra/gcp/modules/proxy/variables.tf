...
variable "image_digest" {
  description = "Immutable image digest (sha256:...) to pin the container to. Sourced from the shared infra/gcp/image.json by the root and bumped by the litomi CI promotion PR."
  type        = string
  nullable    = false

  validation {
    condition     = startswith(var.image_digest, "sha256:")
    error_message = "image_digest must be an immutable digest starting with sha256:."
  }
}

variable "region" {
  description = "Cloud Run region. Must support domain mappings (asia-east1/Taiwan and asia-northeast1/Tokyo do; asia-northeast2/Osaka and asia-northeast3/Seoul do NOT). VPC-unconnected so egress rotates across Google's dynamic IP pool."
  type        = string
