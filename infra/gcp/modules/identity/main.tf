# Bootstrap / identity plane for one GCP project. Owns the HCP Terraform trust
...
#
# Self-lockout guard: the identity workspace runs AS `tf-gcp-bootstrap`, which it
# also manages. The root-of-trust resources (pool, provider, bootstrap SA, and its
# impersonation binding) carry `prevent_destroy` so a bad plan can't revoke the
...
# --- Bootstrap SA: identity the identity workspace runs as (privileged) ---
resource "google_service_account" "bootstrap" {
  project      = var.project_id
  account_id   = var.bootstrap_sa_id
  display_name = "HCP Terraform ${var.bootstrap_workspace_name} bootstrap"
...
resource "google_service_account_iam_member" "bootstrap_wif" {
  ...
  member             = "${local.wif_pool_principal}/attribute.terraform_workspace_name/${var.bootstrap_workspace_name}"
...
# --- Proxy deployer SA: identity the proxy workspace runs as (scoped) ---
resource "google_service_account" "proxy_deployer" {
  project      = var.project_id
  account_id   = var.proxy_deployer_sa_id
  display_name = "HCP Terraform ${var.proxy_workspace_name} deployer"
}
...
resource "google_service_account_iam_member" "proxy_deployer_wif" {
  ...
  role               = "roles/iam.workloadIdentityUser"
  member             = "${local.wif_pool_principal}/attribute.terraform_workspace_name/${var.proxy_workspace_name}"
}
