...
HCP Terraform (org `litomi`, project `gcp`), VPC-unconnected. Deployed to one GCP
account (`proxy.litomi.cc`).
...
```
infra/gcp/
  image.json     # immutable image digest, bumped by litomi CI (the proxy pins it)
  modules/
  ...
  project/       # identity root  → workspace gcp-project
  proxy/         # proxy root     → workspace gcp-proxy   (Tokyo, proxy.litomi.cc)
```

The proxy root is a thin root over the two modules. `region`, `custom_domain`,
...
| ----------- | ------------- | ------------------- | ------------------------ |
| `./project` | `gcp-project` | `infra/gcp/project` | WIF trust + deployer SAs |
| `./proxy`   | `gcp-proxy`   | `infra/gcp/proxy`   | Cloud Run proxy          |

VCS-driven: PRs plan, merges apply.

## Workspace variables

Same across the two workspaces (put in a variable set):

| Key                              | Category  | Value                                                         |
| -------------------------------- | --------- | ------------------------------------------------------------- |
| `project_id`                     | terraform | GCP project id                                                |
| `project_number`                 | terraform | GCP project number                                            |
...
`workloadIdentityUser` binding for workspace `gcp-project`, then apply with the
`TFC_GCP_*` vars from the start.

## First proxy apply

`gcp-proxy` needs no key — it impersonates the `tf-gcp-proxy` SA the identity
...
The image is pinned from `image.json`, so the first apply works without waiting for
a new CI build.

## Image promotion

`litomi` CI builds `ghcr.io/litomi2026/litomi-proxy` (linux/amd64) on each `main`
build and opens a PR here bumping `infra/gcp/image.json` `.digest`. The proxy root
reads that file via `jsondecode(file("../image.json"))`. CI holds no GCP credentials.

The GHCR package must be **Public** — the Artifact Registry pull-through cache
fetches it (Cloud Run cannot pull `ghcr.io` directly, and has no private third-party
registry auth). Set once: GitHub → Packages → `litomi-proxy` → Package settings →
Change visibility → Public.

## Cloudflare

For the proxy hostname (in `infra/cloudflare`):
...
- SSL/TLS `strict`; the mapping issues its own cert.

The Cloud Run host is intentionally not in the Vercel hosts' Turnstile WAF gate
(`rulesets/waf-custom` `edge_proxy_host_expression_set`); add it there only if you
